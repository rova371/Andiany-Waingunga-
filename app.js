...
// En cas de reconnexion automatique (prénom déjà connu), on ne passe pas par
// le clic du bouton "Entrer" : sans geste utilisateur, le téléphone refuse de
// jouer le moindre son. On débloque donc le son au tout premier contact
// (n'importe quel appui) avec l'application, une seule fois.
...
setInterval(checkReminders, 15000); // vérifie toutes les 15 secondes
...
const LOVITAO_COLS = ['mm', 'vkm1', 'vkm2', 'mpiremby', 'talenta', 'date'];
let lovitaoData = {};
...
function renderLovitaoList(){
  ...
  if(entries.length === 0){
    tbody.innerHTML = '<tr><td colspan="8" style="padding:14px; color:var(--text-dim); font-size:0.85rem; border:none;">Aucune ligne pour l’instant — clique sur "+ Ajouter une ligne".</td></tr>';
    return;
  }
...
// ---- Évolution : points forts / points faibles / à améliorer, par personne ----
function renderEvolutionList(){
  ...
  const entries = Object.entries(lovitaoData);
  if(entries.length === 0){
    wrap.innerHTML = '<div style="color:var(--text-dim); font-size:0.85rem;">Ajoute d’abord une ligne dans l’onglet Tableau.</div>';
    return;
  }
  entries.forEach(([id, child]) => {
    ...
    card.innerHTML = `
      <h3>${escapeHtml(child.name || '(sans nom)')}</h3>
      <div class="field"><label>Points forts</label><textarea data-field="fort">${escapeHtml(child.fort || '')}</textarea></div>
      <div class="field"><label>Points faibles</label><textarea data-field="faible">${escapeHtml(child.faible || '')}</textarea></div>
      <div class="field"><label>À améliorer</label><textarea data-field="ameliorer">${escapeHtml(child.ameliorer || '')}</textarea></div>
    `;
    card.querySelectorAll('textarea').forEach(ta => {
      ta.addEventListener('change', () => saveLovitaoCell(id, ta.dataset.field, ta.value));
    });
...
function renderPresenceHead(){
  ...
  if(!row) return;
  row.innerHTML = '<th>Nom</th>' + SUNDAYS.map(dstr => {
...
function renderPresenceBody(){
  ...
  if(entries.length === 0){
    tbody.innerHTML = `<tr><td colspan="${SUNDAYS.length + 1}" style="padding:14px; color:var(--text-dim); font-size:0.85rem; border:none;">Aucun nom pour l’instant — clique sur "+ Ajouter un nom".</td></tr>`;
  } else {
    entries.forEach(([id, person]) => {
      ...
      SUNDAYS.forEach(dateStr => {
        ...
        if(state === true){ td.textContent = '✅'; td.classList.add('present'); present++; }
        else if(state === false){ td.textContent = '❌'; td.classList.add('absent'); absent++; }
        else td.textContent = '—';
        td.addEventListener('click', () => {
          let next;
  ...
  if(stats) stats.textContent = `Présences enregistrées : ${present} · Absences enregistrées : ${absent}`;
}
