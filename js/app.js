...
if(myPhoto) document.getElementById('gatePhoto').value = myPhoto;
function enterApp(){
  const val = document.getElementById('gateName').value.trim();
  if(!val) return;
  myName = val;
  myPhoto = document.getElementById('gatePhoto').value.trim();
...
document.getElementById('chatForm').addEventListener('submit', (e) => {
  ...
  db.collection('messages').add({
    text, author: myName, authorUid: myUid, authorPhoto: myPhoto || null,
...
function listenChat(){
  db.collection('messages').orderBy('createdAt', 'asc').limitToLast(200).onSnapshot((snap) => {
    ...
    snap.forEach(d => {
      ...
      const avatarHtml = m.authorPhoto
        ? `<img class="avatar" src="${escapeHtml(m.authorPhoto)}" alt="" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'avatar-fallback',textContent:'${initial}'}))">`
...
function renderLovitaoList(){
  ...
  if(entries.length === 0){
    list.innerHTML = '<div style="color:var(--text-dim); font-size:0.85rem;">Aucun enfant ajouté pour l’instant.</div>';
    return;
  }
  entries.forEach(([id, child]) => {
    const card = document.createElement('div');
    card.className = 'child-card';
    const chars = child.characteristics || [];
    card.innerHTML = `
      <div class="child-card-head">
        <h4 data-act="rename">${escapeHtml(child.name || '(sans nom)')}</h4>
        <button data-act="delete-child" title="Supprimer">🗑</button>
      </div>
      <div class="child-tags">
        ${chars.map((c, i) => `<span class="char-tag">${escapeHtml(c.label)} : ${escapeHtml(c.value)}<button data-act="remove-char" data-idx="${i}">✕</button></span>`).join('') || '<span style="color:var(--text-dim); font-size:0.8rem;">Aucune caractéristique</span>'}
      </div>
      <div class="child-card-actions">
        <button class="btn ghost small" data-act="add-char">+ Caractéristique</button>
      </div>
    `;
    card.querySelector('[data-act="rename"]').addEventListener('click', () => {
      const newName = prompt('Nouveau nom :', child.name || '');
      if(newName && newName.trim()) db.collection('lovitao').doc(id).update({ name: newName.trim() });
    });
    card.querySelector('[data-act="delete-child"]').addEventListener('click', () => {
      if(confirm(`Supprimer ${child.name} du suivi ?`)) db.collection('lovitao').doc(id).delete();
    });
    card.querySelector('[data-act="add-char"]').addEventListener('click', () => {
      currentChildId = id;
      document.getElementById('charLabel').value = '';
      document.getElementById('charValue').value = '';
      charOverlay.classList.add('open');
    });
    card.querySelectorAll('[data-act="remove-char"]').forEach(btn => {
      btn.addEventListener('click', () => {
        const idx = Number(btn.dataset.idx);
        const newChars = chars.slice(0, idx).concat(chars.slice(idx + 1));
        db.collection('lovitao').doc(id).update({ characteristics: newChars });
      });
    });
...
if(addChildBtn){
  addChildBtn.addEventListener('click', () => {
    const name = prompt('Prénom de l’enfant à suivre :');
    if(name && name.trim()){
      db.collection('lovitao').add({ name: name.trim(), characteristics: [], createdBy: myName });
    }
  });
}
document.getElementById('charCancelBtn').addEventListener('click', () => charOverlay.classList.remove('open'));
charOverlay.addEventListener('click', (e) => { if(e.target === charOverlay) charOverlay.classList.remove('open'); });
document.getElementById('charSaveBtn').addEventListener('click', () => {
  const label = document.getElementById('charLabel').value.trim();
  const value = document.getElementById('charValue').value.trim();
  if(!label || !value || !currentChildId) return;
  const existing = (lovitaoData[currentChildId] && lovitaoData[currentChildId].characteristics) || [];
  db.collection('lovitao').doc(currentChildId).update({
    characteristics: [...existing, { label, value }],
  });
